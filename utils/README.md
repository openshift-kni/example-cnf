...
- [support-images](support-images): projects where you can find the Containerfile required to build some of the images used as build images by the Example CNF images. These images can be found on quay.io/rh-nfv-int and they are publicly available, you only need credentials to access quay.io. The images can be built with the following commands (you need to run it in a RHEL host with a valid RHEL subscription to be able to download the packages installed in the images, and you need a valid quay.io credentials to push it to quay.io):

```
# build images
$ cd support-images
$ podman build dpdk -f dpdk/Containerfile -t "quay.io/rh-nfv-int/dpdk:v0.0.1"
$ podman build ubi8-base-testpmd -f ubi8-base-testpmd/Containerfile -t "quay.io/rh-nfv-int/ubi8-base-testpmd:v0.0.1"
$ podman build ubi10-base-trex -f ubi10-base-trex/Containerfile -t "quay.io/rh-nfv-int/ubi10-base-trex:v0.0.1"
$ podman build ubi10-base-grout -f ubi10-base-grout/Containerfile -t "quay.io/rh-nfv-int/ubi10-base-grout:v0.0.1"
...
$ podman push quay.io/rh-nfv-int/ubi10-base-trex:v0.0.1
